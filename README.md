...
> This repository is the complete consensus core — every module above ships in this release. The
> wider Phi platform (apps, web, and settlement services) is developed separately; each part is
> published only when it is complete, tested, and verifiable.
...
A production validator binary is built with the full build-tag set, which links the live phi-crypto
core and turns on the consensus-critical crypto paths:

```bash
go build -tags "pebbledb phicrypto_cgo voting_snark reauth" ./cmd/phid
```

- `phicrypto_cgo` links the real phi-crypto C-ABI core, so WebAuthn, BBS+, signature, and Semaphore
  verification run live on-chain (`make build-cgo` / `make test-cgo` build and test this path).
- `voting_snark` enables ZK-verified anonymous voting — the zero-knowledge nullifier-derivation proof.
- `reauth` enables the issuer-reattestation identity-recovery path.

The default, tagless build is a deliberate **fail-safe stub, not a runnable node**: it rejects all
cryptographic verification and cannot even construct the node application, so it can never diverge
from a production validator. Enabling a v1 feature is therefore a build-time choice — the production
binary carries the full tag set — never a source change, and the fail-closed defaults stay in place
as a safety net.
...
[Apache License 2.0](./LICENSE) — © 2026 Homaan Smart Data Co. Licensed under the Apache License, Version 2.0.

The Phi protocol and the original source in this repository
