...
func (m *MsgInstitutionRedeem) ValidateBasic() error {
	...
	// Holder consent: strict self-redeem — the signer (admin) must be the holder whose uphi is burned, so an institution operator cannot force-burn a stranger's balance.
	if m.Admin != m.Holder {
...
func (m *MsgRequestFxEntry) ValidateBasic() error {
	...
	// License must be present and bounded — mirrors MsgRegisterInstitution, so an fx applicant cannot onboard with an empty or unbounded license string.
	if m.License == "" {
		return errors.Wrap(ErrInvalidParams, "license cannot be empty")
	}
