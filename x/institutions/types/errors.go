...
var (
	...
	// RBAC, multisig, caps, and anti-replay.
	...
	// fx provenance: an fx institution's mint/redeem must carry fx metadata; a non-fx one must not.
	ErrInvalidFxMetadata = errors.Register(ModuleName, 20, "invalid fx provenance metadata for the institution type")
	// Institution type, deposit proof, KYC tiers, stepped redemption, and fx onboarding.
	...
	// A mint/redeem must carry a non-empty reference so the idempotency marker is always recorded
	// (an empty ref would silently bypass anti-replay).
