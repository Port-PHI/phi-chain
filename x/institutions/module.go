...
// BeginBlock runs two budgeted state-reclamation sweeps (prune stale cap counters, drain removals); neither touches vaults, supply, or the current day.
func (am AppModule) BeginBlock(ctx context.Context) error {
	sdkCtx := sdk.UnwrapSDKContext(ctx)
	am.keeper.PruneStaleCounters(sdkCtx)
	am.keeper.SweepRemovals(sdkCtx)
	return nil
}

// EndBlock asserts the solvency invariants every block (defense-in-depth); a broken invariant halts the node fail-closed.
