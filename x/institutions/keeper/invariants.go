...
// AllInvariants checks every registered (halting) invariant in one pass; excludes mint-within-backing.
...
// SolvencyInvariant is the global solvency invariant (multi-institution model): TotalSupply(phi) * phi_to_toman = sum(vault_balance(toman)) Since supply is denominated in uphi, it is written as a cross-multiplication to avoid fractions: supply_uphi * phi_to_toman == sum(vault_balance) * UphiPerPhi (UphiPerPhi = 1,000,000; with phi_to_toman = 100,000 this equals supply_uphi = 10 * sum(vault_balance).)
...
// BackingShortfallInvariant: each institution's minted phi (vault_balance) <= its own attested reserve.
...
// EmitBackingHealth reports an institution's backing health: when its vault exceeds its attested reserve it sets a per-institution telemetry gauge of the shortfall and emits a backing-shortfall event.
func (k Keeper) EmitBackingHealth(ctx sdk.Context, inst types.Institution) {
	...
	// Int64() panics out of range; saturate the gauge for an (implausibly) huge shortfall.
