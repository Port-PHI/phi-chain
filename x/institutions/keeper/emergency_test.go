...
// at returns the fixture context advanced by the given number of days from its current block time.
...
// TestEmergencyRedemption_NoResetWithinWindow covers the case where governance cannot indefinitely
// restart the redemption halt by toggling Active — re-activation within the day-90 window keeps the
// original started_at, so the stepped relief keeps progressing; only after the window may it reset.
func TestEmergencyRedemption_NoResetWithinWindow(t *testing.T) {
	...
	// Deactivate then re-activate within the day-90 window: started_at must NOT reset.
	...
	// After the full window (>= day 90), a fresh emergency may reset started_at.
...
// The stepped brake relaxes over time: halted before day 30, 200 PHI from day 30, 2,000 PHI from day
// 60 (both cumulative per holder), unlimited from day 90.
func TestEmergencyRedemption_SteppedCaps(t *testing.T) {
	...
	// Activate the emergency at the base time (started_at = now).
	_, err = f.msg.SetEmergencyRedemption(f.ctx, &types.MsgSetEmergencyRedemption{Authority: f.authority, Active: true})
	require.NoError(t, err)

	// Before day 30: halted (default cap 0). Each redeem uses a distinct ref so it reaches the
	// throttle check rather than tripping idempotency.
	require.ErrorIs(t, f.redeemAt(1, "1", "red-1"), types.ErrRedemptionThrottled)

	// From day 30: cumulative 200 PHI = 20,000,000 Toman.
...
func TestEmergencyRedemption_DeactivateLiftsThrottle(t *testing.T) {
	...
	// Governance deactivates → normal (unthrottled) redemption resumes.
