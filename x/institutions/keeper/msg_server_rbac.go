...
// The four sensitive-action messages share an aggregated-approval multisig: an admin signer records an approval against the content hash; the action executes once the threshold is reached, else stays pending.
...
func (k msgServer) RevokeInstitutionRole(goCtx context.Context, msg *types.MsgRevokeInstitutionRole) (*types.MsgRevokeInstitutionRoleResponse, error) {
	...
	// Revoking an ADMIN shrinks the admin set: invalidate pending approvals so earlier sub-threshold ones cannot execute against the lower threshold.
