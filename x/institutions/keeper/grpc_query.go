...
func (k Keeper) Institution(goCtx context.Context, req *types.QueryInstitutionRequest) (*types.QueryInstitutionResponse, error) {
	...
	// Staleness is DERIVED here (not stored), exactly as the mint gate derives it.
	floor := k.GetParams(ctx).MaxAttestationStalenessSeconds
	return &types.QueryInstitutionResponse{
		Institution:               inst,
		AttestationStale:          inst.IsStaleAt(ctx.BlockTime().Unix(), floor),
		EffectiveStalenessSeconds: types.EffectiveStaleness(floor, inst.Params.AutoSuspendRules.MaxVaultAttestationLatencyS),
	}, nil
}
