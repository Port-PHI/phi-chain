...
func (gs GenesisState) Validate() error {
	...
	for _, t := range gs.Templates {
		...
		// Policy must be well-formed and its hash must match; recompute and compare.
		if err := ValidateDisclosurePolicy(t.MessageCount, t.DisclosableIndices); err != nil {
			return fmt.Errorf("template %s: %w", t.Id, err)
		}
		if !bytes.Equal(t.DisclosurePolicyHash, t.PolicyHash()) {
			return fmt.Errorf("template %s: disclosure_policy_hash does not match its declared policy", t.Id)
		}
		templates[t.Id] = t
	}
