...
func (k msgServer) RegisterCredentialTemplate(goCtx context.Context, msg *types.MsgRegisterCredentialTemplate) (*types.MsgRegisterCredentialTemplateResponse, error) {
	...
	// Issuer BBS+ key must be non-empty and bounded; phi-crypto validates the format at proof time.
	...
	if err := types.ValidateDisclosurePolicy(msg.MessageCount, msg.DisclosableIndices); err != nil {
	...
	t := types.CredentialTemplate{
		...
		DisclosableIndices: msg.DisclosableIndices,
	}
	// Policy hash is chain-derived, never accepted from the issuer.
	...
	return &types.MsgRegisterCredentialTemplateResponse{DisclosurePolicyHash: t.DisclosurePolicyHash}, nil
}
...
func (k msgServer) UpdateCredentialTemplate(goCtx context.Context, msg *types.MsgUpdateCredentialTemplate) (*types.MsgUpdateCredentialTemplateResponse, error) {
	...
	// issuer_bbs_pubkey is immutable after registration (rotate via a new template id).
	t.Version++
	t.SchemaHash = msg.SchemaHash
	t.Name = msg.Name
	t.UpdatedAt = ctx.BlockTime().Unix()
	t.MessageCount = msg.MessageCount
	t.DisclosableIndices = msg.DisclosableIndices
	// Version is inside the hash, so any update invalidates a verifier's pinned hash.
	t.DisclosurePolicyHash = t.PolicyHash()
	k.SetTemplate(ctx, t)
	...
	return &types.MsgUpdateCredentialTemplateResponse{
		Version:              t.Version,
		DisclosurePolicyHash: t.DisclosurePolicyHash,
	}, nil
}
...
// AnchorCredential anchors an off-chain VC: issuer controls issuer_did, subject active, template active and version-matched, and the issuer signature over the credential hash verifies via phi-crypto.
...
// SignAgreement records a listed signer's signature over the agreement hash: only listed DIDs, never twice, never past the deadline, only while pending.
