...
phi-chain hand-rolls no cryptography. All signature, WebAuthn, BBS+, and Semaphore verification is
delegated to [phi-crypto](https://github.com/Port-PHI/phi-crypto), whose primitives are thin
wrappers over mature, externally-audited upstream crates. Those upstream audits cover the underlying
primitives; Phi's own integration code is validated through the project's internal security-review
process.

The privacy primitives rest on standard, well-studied assumptions — BBS+ selective disclosure on the
q-SDH assumption over BLS12-381, and nullifier unlinkability on the SXDH / decisional Diffie–Hellman
assumption in the G1 subgroup. Anonymous voting additionally requires the zero-knowledge
nullifier-derivation proof; until that ships and is externally reviewed, the `x/voting` accept path
stays disabled behind the `voting_snark` build tag (it cannot be enabled by governance). Client-side
proving and its side-channel considerations are documented in phi-crypto's security policy.
