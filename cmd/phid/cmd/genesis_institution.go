...
// AddInstitutionCmd returns `phid genesis add-institution`: adds a bootstrap institution whose vault_balance backs the genesis uphi supply so the solvency invariant holds at startup.
...
add-genesis-account). With --operator, this admin is set as the institutions registry operator.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Compute backing to cover the total genesis uphi supply.
			if backSupply {
			...
			instGen.Institutions = append(instGen.Institutions, institutionstypes.Institution{
				...
				PausedMint:      false,
				// Explicit type: UNSPECIFIED (0) fails genesis validation and would skip fx provenance rules.
