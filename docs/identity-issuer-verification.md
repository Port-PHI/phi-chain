...
`did` must be the canonical derivation of `pub_key`:

```
did = "did:phi:" + hex(SHA-256(0x02 ‖ pub_key)[..20])
```

where `0x02` is the P-256 curve tag. This makes the DID self-certifying: a trusted issuer
